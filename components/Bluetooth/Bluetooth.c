...
{
    xEventGroupSetBits(wifi_event_group, BLE_RESP_BIT);
...
void gap_event_handler(esp_gap_ble_cb_event_t event, esp_ble_gap_cb_param_t *param)
{
    ...
    case ESP_GAP_BLE_ADV_START_COMPLETE_EVT:
        ...
        {
            ESP_LOGE("gap_event_handler", "Advertising start failed\n");
        }
        break;
...
static void gatts_profile_a_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param)
{
    ...
    case ESP_GATTS_READ_EVT:
    {
        ...
        else
        {
            ...
            {
                if ((xEventGroupGetBits(wifi_event_group) & BLE_RESP_BIT) == BLE_RESP_BIT)
                {
                    // ble_resp_flag = false;
...
void ble_respon_process(void *arg)
{
    ...
    {
        ESP_LOGI(GATTS_TAG, "ble_respon_process \n");
        if ((xEventGroupWaitBits(wifi_event_group, ACTIVED_BIT, false, true, 5000 / portTICK_RATE_MS) & ACTIVED_BIT) == ACTIVED_BIT)
        {
